...
const marker = `Fictif-QA-${randomUUID().slice(0,8)}`;
...
const runReview = async () => {
  ...
  for(let i=0;i<26;i++)await rpc({operation:'create',command_id:randomUUID(),fields:{first_name:`Page fictive ${i}`,last_name:marker}});
...
try {
  ...
  if (privacyOnly) {
    ...
    for(let i=0;i<26;i++)await rpc({operation:'create',command_id:randomUUID(),fields:{first_name:`Démo ${String(i).padStart(2,'0')}`,last_name:marker}});
    await verifyPagination();
