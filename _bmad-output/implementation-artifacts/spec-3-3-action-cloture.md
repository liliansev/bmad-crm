...
Décisions techniques déléguées préparées après lecture du contrat 3.1, à vérifier sur sa version finale :

- Tâche : `id`, `owner_id`, `opportunity_id` immuable, titre trimé de 1–200 caractères Unicode, `due_date date`, statut `todo|completed|cancelled`, révision, versions titre/date, timestamps création/mise à jour/dernier changement de statut et `completed_at` nullable. FK composite propriétaire/opportunité et index unique partiel par opportunité lorsque `status='todo'`, sans condition d’étape. Achèvement non nul exactement pour `completed`. Prévoir ces colonnes dès 3.3, mais n’exposer complete/cancel/restore autonomes qu’en 3.4.
- Commandes Tâche : create `{command_id,opportunity_id,base_workflow_revision,fields:{title,due_date}}` ; update ajoute `task_id`, patch titre/date non vide et versions ciblées strictement correspondantes. Auth, Zod à HTTP/Action, SQL défensif, refus directwrites et lecture propriétaire. Date civile ISO strictement valide, années 0001–9999 ; ne pas accepter normalisation d’un 31 février ou conversion implicite en instant.
- Verrouillage : clé idempotente propriétaire/commande puis reçu, parent Opportunité FOR UPDATE, workflow_revision comparée, tâches concernées dans un ordre UUID stable. Jamais tâche avant parent. Résultat/reçu et mutations dans la même transaction. Commande réellement mutante : parent workflow_revision et revision augmentées une seule fois, updated_at serveur ; ses versions titre/montant/Notes/liens inchangées. Tâche revision et seules versions ciblées augmentées. Clôture+annulation compte pour une seule mutation workflow.
- Étendre la commande transition 3.1 par `task_resolution?: 'keep'|'cancel'`, sans deuxième chemin. Toute transition réelle vers won/lost avec active, y compris won→lost, exige ce choix. Abandon n’envoie rien. Sans tâche active, ancien format accepté ; nouvelle commande ancien format avec active refusée sans effet. Même étape reste sans mutation ; réouverture n’altère aucune tâche. Une option périmée après autre commande workflow produit conflit explicite, pas choix automatique.
- Compatibilité reçus : consulter et rejouer un reçu antérieur avant toute règle dépendante de l’état actif actuel. Rejouer une transition 3.1 réussie ne doit jamais affecter une tâche créée depuis. Ajouter seulement des propriétés facultatives aux réponses historiques si nécessaire, sans enrichir/réécrire les reçus existants. Migration additive, anciens wrappers privés non exécutables directement par authenticated/anon, aucune voie contournant la clôture atomique.
- UI : contrôleur de transition unique carte/panneau ; choix avant envoi, étape confirmée tant que choix ouvert. Dialogues tâche montés ; état parent active inconnu/erreur interdit un faux « aucune tâche ». Brouillons et commandes exactes propriétaire/génération/bases/pending récupérables ; préserver les corrections 2.5/3.1 contre rebase silencieux, réponse tardive et suppression stockage échouée. Le résultat canonique tâche+opportunité invalide cartes/panneau sans écraser la génération suivante. Créneau DB/navigateur exclusif et fixtures tracées.
- Tests frontières : double création simultanée ; tâche/clôture concurrentes ; Notes ou montant simultanés indépendants ; keep/cancel/abandon carte et panneau ; ancienne commande sans choix HTTP et RPC ; ancien reçu de transition rejoué après création d’une tâche ; clé autre contenu ; dates civiles invalides/past allowed ; droits et relations étrangères. Réponse perdue, stockage indisponible et parent Save/Close sont exercés réellement. Reporter le cycle autonome et historique 10/page à 3.4.
