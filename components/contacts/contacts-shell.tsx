...
import type { Contact, ContactReadResult, ContactsPage } from "@/lib/validations/contacts";
...
export function ContactsShell({ ownerId, initial, initialPage, initialPanel }: Props) {
  ...
  const [cache] = useState(() => { const value = createContactsCache(ownerId); if (initial.status === "success") initial.contacts.forEach(value.put); return value; });
  ...
  const refreshList = useCallback(async (requested = pageRef.current) => {
    ...
    if (result.status === "success") {
      result.contacts.forEach(cache.put);
  ...
  useEffect(() => {
    ...
    return () => { lifecycle.current++; listGeneration.current++; panelGeneration.current++; window.removeEventListener("crm-session-expired", expired); window.removeEventListener("crm-session-ready", refresh); window.removeEventListener("online", refresh); window.removeEventListener("popstate", pop); cache.clear(); };
  ...
  return <>
    ...
    {list.status !== "success" ? <div className="rounded-md border p-6" role="alert"><p>{list.message}</p><Button className="mt-4 min-h-11" onClick={() => refreshList().catch(() => undefined)}>Réessayer</Button></div> : list.total === 0 ? <div className="rounded-md border px-6 py-12 text-center"><h2 className="font-medium">Aucun contact pour le moment</h2><p className="mt-2 text-sm text-muted-foreground">Commencez avec un prénom ou un nom.</p><Button className="mt-5 min-h-11" variant="outline" onClick={() => open("new")}>Ajouter votre premier contact</Button></div> : <>
      <div className="overflow-hidden rounded-md border" aria-busy={loading}><Table><TableHeader><TableRow className="bg-muted/60 hover:bg-muted/60"><TableHead className="w-1/2 px-4">Prénom</TableHead><TableHead className="px-4">Nom</TableHead></TableRow></TableHeader><TableBody>{list.contacts.map((item) => <TableRow key={item.id} data-contact-id={item.id} className="cursor-pointer hover:bg-accent/60" onMouseEnter={() => { prefetch(item.id); router.prefetch("/contacts"); }} onClick={() => open(item.id)}><TableCell className="max-w-0 px-4 py-0"><Button variant="ghost" className="h-auto min-h-11 w-full justify-start px-0 text-left font-normal hover:bg-transparent" onFocus={() => prefetch(item.id)} onClick={(event) => { event.stopPropagation(); open(item.id); }} aria-label={`Ouvrir ${[item.first_name, item.last_name].filter(Boolean).join(" ")}`}><span className="truncate">{item.first_name || "—"}</span></Button></TableCell><TableCell className="max-w-0 truncate px-4">{item.last_name || "—"}</TableCell></TableRow>)}</TableBody></Table></div>
